...
const store = {
  ...
  addNewItem: function(item) {
    ...
    // pkt 3 dodaj walidację na store.addNewItem, aby nie można było dodać produktu bez nazwy //
  ...
  filterItems: function(filter) {
    ...
    // pkt 1 store.filter //
          ...
          .filter(
            item =>
              !filter.shops || filter.shops.length === 0 || item.shops.some(i => filter.shops.find(f => f === i))
          );
